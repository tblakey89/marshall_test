ActiveAdmin.register_page "Dashboard" do
  ...
  ActiveAdmin::Dashboards.build do
    section "Dealership Scores" do
      table_for Dealership.find(:all, select: "dealerships.name, dealerships.id, avg(users.score) as average_score, avg(users.time_taken) as average_time", limit: 10, joins: :users, order: 'average_score', group: "dealerships.name, dealerships.id, average_score, average_time") do |t|
        t.column ("Dealership") { |dealership| link_to dealership.name, admin_dealership_path(dealership) }
        t.column ("Average Score") { |dealership| dealership.average_score.round(2) }
